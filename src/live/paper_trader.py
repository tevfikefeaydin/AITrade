...
class PaperTrader:
    ...
    def __init__(
        ...
        self.positions = PositionManager(str(config.OUTPUTS_DIR / f"{symbol}_paper_trades.json"))

        # Model (loaded on start)
        self.model = None

        # Guard state persistence file
        self._guard_state_path = config.OUTPUTS_DIR / f"{symbol}_guard_state.json"
    ...
    async def _open_position(self, price: float, prob: float, signal_time: datetime):
        ...
        actual_sl_pct = (price - sl_price) / price * 100
        ...
        print(f"  Take Profit:   ${tp_price:.2f} (+{actual_tp_pct:.2f}%)")
        print(f"  Stop Loss:     ${sl_price:.2f} (-{actual_sl_pct:.2f}%)")
